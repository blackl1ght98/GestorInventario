...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modalEl = document.getElementById('confirmEntregaModal');
    const modal = new bootstrap.Modal(modalEl);

    const resultBox = document.getElementById('entregaResult');
    const confirmBtn = document.getElementById('confirmEntregaBtn');
    ...
    confirmBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`/Pedidos/ConfirmarEntrega?pedidoId=${pedidoId}`, {
                method: 'POST',
                headers: {
            ...
            resultBox.textContent = '✔️ Pedido confirmado con exito';

            currentButton.disabled = true;
            currentButton.classList.replace('btn-outline-warning', 'btn-outline-secondary');
            currentButton.textContent = 'Confirmado';
            ...
            resultBox.textContent = '❌ Error al confirmar el pedido';
