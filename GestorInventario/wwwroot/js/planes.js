...
document.getElementById("confirmUpdatePlanPriceBtn").addEventListener("click", function () {
    ...
    console.log("Enviando solicitud de actualización de precio para Plan ID:", planId, "Trial Price:", trialPrice, "Regular Price:", regularPrice);
    ...
    actualizarPrecioPlan(planId, trialPrice, regularPrice)
        .finally(() => {
...
function actualizarPrecioPlan(planId, trialPrice, regularPrice) {
    ...
    return fetch('/Paypal/ActualizarPrecioPlan', {
        ...
        .then(data => {
            if (data.success) {
                console.log(data.message || "Precio del plan actualizado con éxito.");
