...
namespace GestorInventario.Application.Services
{
    public class PaypalServices : IPaypalService
    {
        ...
        public async Task<string> CreateOrderAsync(Checkout pagar)

        {

            try
            {
                // Crea el objeto de solicitud para la orden en la API v1
                var requestData = new
                {
                    intent = "sale",
                    //Para declarar un elemento hijo se hace de esta forma
                    payer = new
                    {
                        payment_method = "paypal"
                    },
                    //Como transactions es un array de objetos aqui se tiene que poner como array, para ello se pone el nombre de la propiedad
                    //el nombre tiene que ser igual al que esta en el json de paypa y al ser un array de objetos se pone: new[]
                    transactions = new[]
                    {
                     // Se crea un objeto anónimo dentro del array, se usa cuando es un objeto complejo, aunque cuando se mande a paypal el onjeto anonimo no 
                     //se manda
                    new
                    {
                    //Este es un objeto hijo que
                    amount = new
                    {
                        total = pagar.totalAmount.ToString("0.00", CultureInfo.InvariantCulture),
                        currency = pagar.currency,
                        details = new
                        {
                            subtotal = pagar.totalAmount.ToString("0.00", CultureInfo.InvariantCulture),
                            tax = "0.00",
                            shipping = "0.00",
                            handling_fee = "0.00",
                    ...
                    invoice_number = Guid.NewGuid().ToString(), // Genera un invoice_id único
                    payment_options = new
                    {
                    ...
                    item_list = new
                    {
                        ...
                        //Implementar formulario de envio por ahora esto estar como dato crudo
                        shipping_address = new
                        {
                            recipient_name = pagar.nombreCompleto,
                            line1 = pagar.line1,
                            line2 = pagar.line2,
                            city = pagar.ciudad,
                            country_code = "US",
                            postal_code = pagar.codigoPostal,
                            phone = pagar.telefono,
                            state = "CA"
                        }
                    }
                }
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    var response = await httpClient.PostAsync("https://api-m.sandbox.paypal.com/v1/payments/payment", content);
        ...
        //    }
        //}
        public async Task<(List<Plan> plans, bool HasNextPage)> GetSubscriptionPlansAsync(int page = 1, int pageSize = 10)
        {
            var authToken = await GetAccessTokenAsync();
