...
namespace GestorInventario.Infraestructure.Controllers
{
    public class PaypalController : Controller
    {
        ...
        public PaypalController(GestorInventarioContext context, IUnitOfWork unit, GenerarPaginas generar, PolicyHandler policy, ILogger<PaypalController> logger, IPaypalController paypalController, ICarritoRepository carritoRepository)
        {
            _context = context;
            _unitOfWork = unit;
        ...
        public async Task<IActionResult> MostrarProductos(int pagina = 1)
        {
            ...
            var paginacion = new Paginacion
            {
                ...
                PaginaActual = pagina,
                Radio = 3 // Aqui puedes ajustar los productos que se mostraran por pagina
            };
        ...
        //Metodo para mostrar los planes al usuario
        public async Task<IActionResult> MostrarPlanes(int pagina = 1)
        {
            int pageSize = 10;
            var (plans, hasNextPage) = await _unitOfWork.PaypalService.GetSubscriptionPlansAsync(pagina, pageSize);

            // Mapear la lista de Plan a PlanesViewModel
            var planesViewModel = plans.Select(plan => new PlanesViewModel
            {
                id = plan.id,
                ...
                billing_cycles = plan.billing_cycles?.Select(cycle => new BillingCycle
                {
                    frequency = new FrequencyPlans
                    {
                    ...
                    pricing_scheme = new PricingScheme
                    {
                        ...
                        status = cycle.pricing_scheme.status,
                ...
                Taxes = plan.taxes != null ? new Taxes
                {
                    percentage = plan.taxes.percentage,
                    inclusive = plan.taxes.inclusive
                } : null
            }).ToList();
