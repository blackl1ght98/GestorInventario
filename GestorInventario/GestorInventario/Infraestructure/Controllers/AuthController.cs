...
namespace GestorInventario.Infraestructure.Controllers
{
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            try
            {
                // Comprobar si el User-Agent es válido
                var userAgent = Request.Headers["User-Agent"].ToString();
                if (string.IsNullOrEmpty(userAgent) || userAgent.Contains("bot", StringComparison.OrdinalIgnoreCase))
                {
                    _logger.LogWarning("Acceso denegado por User-Agent sospechoso: {UserAgent}", userAgent);
                    return BadRequest("Acceso denegado para bots.");
                }
        ...
        //Conjunto de 3 metodos que es para restablecer la contraseña del usuario, la diferencia es que este
        //metodo si lo puede hacer el usuario y no requiere que el admin intervenga
